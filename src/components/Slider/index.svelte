<script>
    ...
    $: deltaFillLeft = variant === 'delta' 
        ? (percentage < defaultPercentage ? percentage : defaultPercentage)
        : 0;
    $: deltaFillWidth = variant === 'delta' 
        ? Math.abs(percentage - defaultPercentage)
        : percentage;
    $: fillHasValue = variant === 'delta'
        ? deltaFillWidth > 0
        : percentage > 0;

    // Calculate tick marks for stepper variant
    $: tickCount = variant === 'stepper' ? Math.floor((max - min) / step) + 1 : 0;
    $: tickPositions = variant === 'stepper' ? Array.from({ length: tickCount }, (_, i) => {
...
<div class="slider-container {className}" class:disabled>
    <div class="slider-track" class:disabled>
        ...
        {#if variant === 'delta'}
            ...
            <div 
                class="slider-default-indicator"
                class:disabled
