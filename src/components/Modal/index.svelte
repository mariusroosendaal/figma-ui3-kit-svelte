<script>
    ...
    $: modalWidth = (() => {
        ...
        if (position === 'bottom' && typeof width === 'string' && ['small', 'medium', 'large'].includes(width)) {
        ...
        if (typeof width === 'string') {
            switch (width) {
                ...
                default: return width; // Custom string value
            }
        }
    ...
    $: modalHeight = (() => {
        if (typeof height === 'string') {
            switch (height) {
                ...
                case '50vh': return '50vh';
                case '80vh': return '80vh';
                default: return height; // Custom string value
            }
        }
...
{#if isOpen}
    {#if showOverlay}
        <div class="modal-overlay modal-overlay--{position}" style="padding: {overlayPadding}" on:click={handleOverlayClick}>
            <div 
                class="modal-container modal-container--{position} {className}" 
                style="width: {modalWidth}; height: {modalHeight || 'auto'}; max-height: {modalMaxHeight}; border-radius: {modalBorderRadius}"
...
<style>
    ...
    .modal-container {
        ...
        box-shadow: 0 0 0.5px rgba(0,0,0,0.08), 0 2px 5px rgba(0,0,0,0.15), 0 10px 24px rgba(0,0,0,0.18);
        display: flex;
